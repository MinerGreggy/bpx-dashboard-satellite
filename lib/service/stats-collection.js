...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'bpx-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('bpx').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('bpx')[service]);
